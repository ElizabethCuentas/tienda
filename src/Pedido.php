...
class Pedido{
    ...
    public function registrarDetalle($_params){
        $sql = "INSERT INTO `detalles_pedido`(`pedidos_id`, `pelicula_id`, `precio`,`cantidad`)
    ...
    public function mostrar()
{
    ...
    INNER JOIN cliente c ON p.cliente_id = c.id ORDER BY p.id DESC";
...
{
    $sql = "SELECT p.id, nombre, apellidos, email, total, fecha FROM pedidos p
    INNER JOIN cliente c ON p.cliente_id = c.id WHERE p.id = :id";
...
public function mostrarDetallePorIdPedido($id)
{
    $sql = "SELECT
            ...
            INNER JOIN pelicula pe ON pe.id= dp.pelicula_id
...
{
$sql = "SELECT p.id, nombre, apellidos, email, total, fecha FROM pedidos p
INNER JOIN cliente c ON p.cliente_id = c.id ORDER BY p.id DESC LIMIT 10";
